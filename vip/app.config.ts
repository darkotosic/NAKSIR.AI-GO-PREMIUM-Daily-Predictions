...
export default ({ config }: ConfigContext): ExpoConfig => {
  const androidPackage =
    readEnv('EXPO_PUBLIC_ANDROID_PACKAGE') ?? 'com.naksir.soccerpredictions.vip';
  ...
  const base: ExpoConfig = {
    ...
    ios: {
      bundleIdentifier: 'com.naksir.soccerpredictions.vip',
    },
    plugins: [
      [
  ...
  const mergedExtra = {
    ...(base.extra ?? {}),
    appId: readEnv('EXPO_PUBLIC_APP_ID') ?? 'naksir.vip',
