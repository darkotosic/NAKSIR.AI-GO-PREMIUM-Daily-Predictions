...
import VipRequiredCard from '@components/VipRequiredCard';
...
const AIAnalysisScreen: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (entitlementsLoading || !isPremium) {
      return () => stopPolling();
    }

    startGeneration({ live: isLiveMatch });
    return () => stopPolling();
  }, [fixtureId, isLiveMatch, startGeneration, stopPolling, entitlementsLoading, isPremium]);
  ...
  return (
    <SafeAreaView style={styles.safeArea}>
      {entitlementsLoading ? (
        <LoadingState message="Checking VIP access..." />
      ) : !isPremium ? (
        <VipRequiredCard
          onGoToPlans={() => navigation.navigate('Subscriptions')}
          onBackToMatches={() => navigation.navigate('TodayMatches')}
        />
      ) : (
